...
public sealed class ChatGptProvider : IUsageProvider
{
    ...
    public async Task<UsageSnapshot> FetchAsync(WebViewUsageHost host)
    {
        ...
        AddWindowBar(bars, rateLimit, "primary_window", Strings.T("provider.weekly"), isPrimary: true);
        AddWindowBar(bars, rateLimit, "secondary_window", Strings.T("provider.chatgpt.short"), isPrimary: false);
    ...
    private static void AddWindowBar(List<UsageBar> bars, JsonElement rateLimit, string propertyName, string label, bool isPrimary)
    {
        ...
        bars.Add(new UsageBar { Label = label, Percent = percent, ResetAt = resetAtValue, IsPrimary = isPrimary });
    }
}
