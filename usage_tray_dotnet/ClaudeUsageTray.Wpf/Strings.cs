namespace ClaudeUsageTray;

public enum AppLanguage
{
    Spanish,
    English,
}

/// <summary>
/// A flat key→text lookup rather than .resx, since almost all of this app's
/// UI is built in code (not XAML) — a plain dictionary is far less
/// ceremony than wiring up culture-specific resource assemblies for what
/// amounts to two languages. <see cref="Current"/> is process-global and
/// mutated in place (not per-window state) so every already-open window,
/// the tray context menu, and anything built fresh afterward all agree —
/// see TrayOrchestrator's language preview/apply for how live switching
/// is wired up.
/// </summary>
internal static class Strings
{
    public static AppLanguage Current { get; set; } = AppLanguage.Spanish;

    public static string T(string key) =>
        (Current == AppLanguage.Spanish ? Es : En).TryGetValue(key, out var v) ? v : key;

    public static string F(string key, params object[] args) => string.Format(T(key), args);

    private static readonly Dictionary<string, string> Es = new()
    {
        ["app.name"] = "Uso de IA",

        ["settings.title"] = "Ajustes",
        ["settings.cancel"] = "Cancelar",
        ["settings.save"] = "Guardar",
        ["settings.version.tooltip"] = "Acerca de",

        ["card.general"] = "General",
        ["card.update"] = "Actualización",
        ["card.popup"] = "Panel emergente",
        ["card.appearance"] = "Apariencia",
        ["card.services"] = "Servicios",
        ["card.telegram"] = "Bot de Telegram",
        ["card.debugging"] = "Depuración",

        ["general.autostart.label"] = "Iniciar con Windows",
        ["general.autostart.hint"] = "Abre la app automáticamente al encender el equipo",
        ["general.animations.label"] = "Animaciones",
        ["general.animations.hint"] = "Anima las barras de progreso del panel emergente",
        ["general.language.label"] = "Idioma",
        ["general.autoupdate.label"] = "Buscar actualizaciones automáticamente",
        ["general.autoupdate.hint"] = "Si lo desactivas, puedes comprobarlo tú mismo desde el enlace de versión de abajo",

        ["update.frequency.label"] = "Frecuencia de actualización (en minutos)",

        ["popup.mode.tooltip"] = "Tooltip sencillo",
        ["popup.mode.rich"] = "Ventana flotante (recomendado)",
        ["popup.mode.hoverhint"] = "Mostrar panel al pasar el ratón por encima",
        ["hoverdelay.instant"] = "Instantáneo",

        ["theme.system"] = "Sistema",
        ["theme.light"] = "Claro",
        ["theme.dark"] = "Oscuro",
        ["appearance.accent.hint"] = "Color de acento — \"Original\" colorea las barras del panel según el % de uso",
        ["appearance.palette.hint"] = "Paleta — solo afecta a esta ventana y a Estadísticas; el panel flotante no cambia",
        ["appearance.windowstyle.hint"] = "Estilo de la ventana del panel (experimental)",
        ["appearance.hoverglow.label"] = "Brillo al pasar el cursor",
        ["appearance.hoverglow.hint"] = "Un resplandor sutil que sigue al cursor sobre los botones de Ajustes y Estadísticas",
        ["appearance.windowstyle.standard"] = "Ventana estándar",
        ["appearance.windowstyle.blur"] = "Ventana con blur",
        ["appearance.opacity.label"] = "Opacidad de la ventana",
        ["appearance.blur.label"] = "Transparencia del cristal",

        ["service.loggedin"] = "Sesión iniciada",
        ["service.link"] = "Vincular cuenta",
        ["service.sound.label"] = "Recibir notificación con sonido",
        ["service.sound.hint"] = "Se te notificará con sonido cuando se reinicie el uso",
        ["service.notify.tooltip"] = "Avisar cuando se reinicie el límite o cuando se agote",

        ["service.compact.title"] = "Vista compacta",
        ["service.compact.hint"] = "Qué servicios se muestran cuando el panel está en modo compacto",

        ["telegram.enable.label"] = "Activar bot de Telegram",
        ["telegram.linked"] = "Chat vinculado",
        ["telegram.linked.hint"] = "Escríbele /uso al bot en cualquier momento para consultar tu consumo actual de todos los servicios activos, directamente desde Telegram.",
        ["telegram.setup.title"] = "Para vincular el bot:",
        ["telegram.setup.1"] = "1. Abre Telegram y busca a @BotFather.",
        ["telegram.setup.2"] = "2. Envíale /newbot y sigue los pasos para crear tu bot.",
        ["telegram.setup.3"] = "3. Pega aquí el token que te entregue y guarda los ajustes.",
        ["telegram.setup.4"] = "4. Escríbele /uso a tu nuevo bot para vincular el chat.",
        ["telegram.notifyusage.label"] = "Recibir notificaciones de uso por Telegram",
        ["telegram.notifyusage.hint"] = "Te avisará por Telegram cuando se reinicie un límite o se agote, igual que en el escritorio",
        ["telegram.notify80.label"] = "Recibir también avisos al 80%",
        ["telegram.notify80.hint"] = "Además te avisará por Telegram en cuanto un servicio llegue al 80% de uso",
        ["telegram.notify80.message"] = "Ojo, que tienes {0} al 80% 🫣",

        ["popup.noservices"] = "No hay servicios activos. Ábrelos desde Ajustes.",
        ["popup.error.generic"] = "No se pudo leer el uso",
        ["popup.updated"] = "Actualizado {0}",
        ["popup.resets"] = "Se reinicia {0}",
        ["popup.loading"] = "Actualizando…",
        ["popup.loading.progress"] = "{0} ({1}/{2})",
        ["popup.tooltip.refresh"] = "Actualizar",
        ["popup.tooltip.settings"] = "Ajustes",
        ["popup.tooltip.stats"] = "Estadísticas",
        ["popup.tooltip.close"] = "Cerrar panel",
        ["popup.tooltip.pin"] = "Fijar panel (no se cerrará solo)",
        ["popup.tooltip.compact"] = "Vista compacta",
        ["popup.tooltip.compact.full"] = "Vista completa",
        ["popup.compact.none"] = "No has elegido ningún servicio para la vista compacta. Actívalo en Ajustes.",

        ["stats.title"] = "Estadísticas",
        ["stats.maximize"] = "Maximizar",
        ["stats.restore"] = "Restaurar",
        ["stats.subtitle"] = "Últimas horas",
        ["stats.range.today"] = "Hoy",
        ["stats.range.yesterday"] = "Ayer",
        ["stats.range.week"] = "Semana",
        ["stats.range.month"] = "Mes",
        ["stats.range.custom"] = "Otra fecha",
        ["stats.calendarview"] = "Vista de calendario",
        ["stats.promptmode.new"] = "Nuevos",
        ["stats.promptmode.total"] = "Totales",
        ["stats.empty"] = "Aún no hay suficientes datos. Vuelve más tarde para ver la evolución de tu uso.",
        ["stats.reset"] = "✨ Reseteo",
        ["stats.prompts.legend"] = "prompts (máx {0})",
        ["stats.prompts.hover"] = "{0} prompts",
        ["stats.dashboard.prompts"] = "{0} prompts",
        ["stats.dashboard.projects"] = "{0} proyectos",
        ["stats.dashboard.tasks"] = "{0} tareas",
        ["stats.dashboard.totals.title"] = "Totales (histórico completo)",
        ["stats.dashboard.totals.badge"] = "Totales",
        ["stats.legend.usage"] = "Uso de IA",
        ["stats.legend.prompts"] = "Prompts",
        ["stats.noservices"] = "No hay servicios activos.",

        ["tray.tooltip.starting"] = "{0}: iniciando...",
        ["tray.tooltip.noservices"] = "{0}: no hay servicios activos (clic derecho → Ajustes)",
        ["menu.refresh"] = "Actualizar ahora",
        ["menu.settings"] = "Ajustes...",
        ["menu.about"] = "Acerca de...",
        ["menu.login"] = "Iniciar sesión",
        ["menu.exit"] = "Salir",
        ["loading"] = "Cargando...",

        ["toast.reset"] = "El uso de {0} se ha reseteado ✨{1}",
        ["toast.exhausted"] = "Se te ha gastado {0} 😭{1}",

        ["dialog.yes"] = "Sí",
        ["dialog.no"] = "No",
        ["dialog.later"] = "Hoy no, mañana",
        ["dialog.ok"] = "Vale",
        ["dialog.update.title"] = "Actualización disponible",
        ["dialog.update.message"] = "Hay una nueva versión disponible (v{0}).\n¿Quieres actualizarla ahora?",

        ["about.version"] = "Versión {0}",
        ["about.path.label"] = "Ruta del archivo",
        ["about.changelog.label"] = "Novedades",
        ["about.checkupdate.link"] = "Buscar actualizaciones",
        ["about.checkupdate.checking"] = "Buscando actualizaciones...",
        ["dialog.checkfailed.message"] = "No se ha podido comprobar si hay actualizaciones.",
        ["dialog.ratelimited.message"] = "GitHub está limitando temporalmente las comprobaciones de actualización. Inténtalo de nuevo dentro de un rato.",
        ["dialog.toosoon.message"] = "Ya se ha comprobado hace un momento. Inténtalo de nuevo en unos segundos.",
        ["dialog.uptodate.message"] = "Ya tienes la última versión.",

        ["debug.save.label"] = "Guardar datos de diagnóstico",
        ["debug.save.hint"] = "Registra en tu equipo la forma exacta de los datos que llegan de Claude/ChatGPT/Grok en cada actualización — solo se usa para investigar fallos como uno que muestre mal un límite; no se envía a ningún sitio salvo que tú lo compartas con el botón de abajo.",
        ["debug.share.button"] = "Guardar y compartir logs",
        ["debug.clear.button"] = "Borrar datos de diagnóstico",
        ["debug.share.nofiles.title"] = "Sin datos de diagnóstico",
        ["debug.share.nofiles.message"] = "Todavía no hay ningún archivo de diagnóstico guardado.",
        ["debug.share.error.title"] = "No se pudo preparar el archivo",
        ["debug.share.error.message"] = "No se ha podido preparar el archivo para compartir:\n{0}",
        ["debug.share.fallback.title"] = "Archivo listo",
        ["debug.share.fallback.message"] = "No se pudo abrir el panel de compartir de Windows, pero el archivo está listo aquí para que lo compartas a mano:\n{0}",
        ["debug.clear.confirm.title"] = "Borrar datos de diagnóstico",
        ["debug.clear.confirm.message"] = "Se borrarán todos los archivos de diagnóstico guardados en este equipo. Esto no afecta a tu cuenta ni a tus ajustes. ¿Continuar?",
        ["debug.clear.done.title"] = "Hecho",
        ["debug.clear.done.message"] = "Datos de diagnóstico borrados.",
        ["debug.clear.error.message"] = "No se han podido borrar del todo:\n{0}",

        ["error.startup.body"] = "No se ha podido iniciar la aplicación.",
        ["error.runtime.body"] = "Ha ocurrido un error inesperado. La aplicación seguirá abierta, pero algo puede no haber funcionado bien.",
        ["error.logpath"] = "Detalles guardados en:\n{0}",
        ["startup.folder.readonly.body"] = "Esta carpeta no tiene permisos de escritura para tu usuario:\n{0}\n\nEsto va a impedir que la aplicación se actualice sola (y puede afectar a otras funciones, como el registro de estadísticas). Mueve la aplicación a otra carpeta — por ejemplo, reinstalándola con el instalador en su carpeta por defecto — o ejecútala como administrador.",

        ["time.available"] = "ya disponible",
        ["time.ago.moment"] = "hace un momento",
        ["time.ago.seconds"] = "hace unos segundos",
        ["time.ago.minutes"] = "hace {0} min",
        ["time.ago.hours"] = "hace {0} h",
        ["time.ago.days"] = "hace {0} d",

        ["provider.claude.5h"] = "Límite de 5 horas",
        ["provider.weekly"] = "Límite semanal",
        ["provider.claude.fable"] = "Fable",
        ["qualifier.weekly"] = "semanal",
        ["qualifier.5h"] = "5 horas",
        ["qualifier.usage"] = "uso",
        ["qualifier.fable"] = "Fable",
        ["prefix.weekly"] = "S:",
        ["prefix.5h"] = "5H:",
        ["prefix.fable"] = "F:",
        ["prefix.usage"] = "U:",
        ["provider.grok.usage"] = "Límite de uso",
        ["provider.timeout"] = "Tiempo de espera agotado",
        ["provider.grok.loginneeded"] = "Inicia sesión en la ventana de Grok",
        ["provider.chatgpt.credits"] = "Saldo de créditos: {0:0}",
        ["provider.chatgpt.resets"] = "Cuota reset: {0}",
        ["provider.chatgpt.weekly_general"] = "Semanal general",
        ["provider.chatgpt.weekly_model"] = "Semanal {0}",
        ["qualifier.chatgpt.weekly_general"] = "semanal general",
        ["qualifier.chatgpt.weekly_model"] = "semanal {0}",
        ["prefix.chatgpt.weekly_model"] = "S+",
        ["provider.claude.credits.used_of"] = "Créditos usados: {0:0.00} / {1:0.00} {2}",

        ["telegrambot.prompt"] = "Pulsa el botón o escribe /uso para ver tu consumo.",
        ["telegrambot.none"] = "No hay servicios activos todavía.",
        ["telegrambot.nodata"] = "sin datos",
        ["telegrambot.usebutton"] = "Usa el botón «{0}» o el comando /uso.",
        ["telegrambot.apps.title"] = "🖥️ Apps en uso",
        ["telegrambot.apps.none"] = "No se ha detectado ninguna app con ventana abierta.",
        ["telegrambot.apps.foreground"] = "primer plano",
        ["telegrambot.apps.active"] = "activa ({0}% CPU)",
        ["telegrambot.apps.footer"] = "{0} apps abiertas",
        ["telegrambot.projects.none"] = "No se ha encontrado historial de Claude Code en este PC.",
        ["telegrambot.projects.active"] = "ahora",
        ["telegrambot.projects.prompts"] = "{0} prompts",
        ["telegrambot.shutdown.confirm"] = "⚠️ ¿Seguro que quieres *apagar* el PC?",
        ["telegrambot.restart.confirm"] = "⚠️ ¿Seguro que quieres *reiniciar* el PC?",
        ["telegrambot.confirm.yes"] = "✅ Sí",
        ["telegrambot.confirm.no"] = "❌ Cancelar",
        ["telegrambot.cancelled"] = "Cancelado.",
        ["telegrambot.shutdown.doing"] = "🔴 Apagando el PC…",
        ["telegrambot.restart.doing"] = "🔁 Reiniciando el PC…",
    };

    private static readonly Dictionary<string, string> En = new()
    {
        ["app.name"] = "AI Usage",

        ["settings.title"] = "Settings",
        ["settings.cancel"] = "Cancel",
        ["settings.save"] = "Save",
        ["settings.version.tooltip"] = "About",

        ["card.general"] = "General",
        ["card.update"] = "Updates",
        ["card.popup"] = "Popup panel",
        ["card.appearance"] = "Appearance",
        ["card.services"] = "Services",
        ["card.telegram"] = "Telegram bot",
        ["card.debugging"] = "Debugging",

        ["general.autostart.label"] = "Start with Windows",
        ["general.autostart.hint"] = "Opens the app automatically when your PC starts",
        ["general.animations.label"] = "Animations",
        ["general.animations.hint"] = "Animates the popup panel's progress bars",
        ["general.language.label"] = "Language",
        ["general.autoupdate.label"] = "Automatically check for updates",
        ["general.autoupdate.hint"] = "Turn this off and you can still check manually from the version link below",

        ["update.frequency.label"] = "Refresh frequency (in minutes)",

        ["popup.mode.tooltip"] = "Simple tooltip",
        ["popup.mode.rich"] = "Floating window (recommended)",
        ["popup.mode.hoverhint"] = "Show panel on hover",
        ["hoverdelay.instant"] = "Instant",

        ["theme.system"] = "System",
        ["theme.light"] = "Light",
        ["theme.dark"] = "Dark",
        ["appearance.accent.hint"] = "Accent color — \"Original\" colors the panel bars based on usage %",
        ["appearance.palette.hint"] = "Palette — only affects this window and Stats; the floating panel doesn't change",
        ["appearance.windowstyle.hint"] = "Panel window style (experimental)",
        ["appearance.hoverglow.label"] = "Button hover glow",
        ["appearance.hoverglow.hint"] = "A subtle glow that follows the cursor over Settings and Stats buttons",
        ["appearance.windowstyle.standard"] = "Standard window",
        ["appearance.windowstyle.blur"] = "Blur window",
        ["appearance.opacity.label"] = "Window opacity",
        ["appearance.blur.label"] = "Glass transparency",

        ["service.loggedin"] = "Signed in",
        ["service.link"] = "Link account",
        ["service.sound.label"] = "Play a sound with notifications",
        ["service.sound.hint"] = "You'll hear a sound when your usage resets",
        ["service.notify.tooltip"] = "Notify when usage resets or runs out",

        ["service.compact.title"] = "Compact view",
        ["service.compact.hint"] = "Which services show up when the panel is in compact mode",

        ["telegram.enable.label"] = "Enable Telegram bot",
        ["telegram.linked"] = "Chat linked",
        ["telegram.linked.hint"] = "Message /uso to the bot any time to check your current usage across every active service, right from Telegram.",
        ["telegram.setup.title"] = "To link the bot:",
        ["telegram.setup.1"] = "1. Open Telegram and search for @BotFather.",
        ["telegram.setup.2"] = "2. Send it /newbot and follow the steps to create your bot.",
        ["telegram.setup.3"] = "3. Paste the token it gives you here and save your settings.",
        ["telegram.setup.4"] = "4. Message /uso to your new bot to link the chat.",
        ["telegram.notifyusage.label"] = "Get usage notifications on Telegram",
        ["telegram.notifyusage.hint"] = "You'll be notified on Telegram whenever a limit resets or runs out, same as on the desktop",
        ["telegram.notify80.label"] = "Also notify at 80%",
        ["telegram.notify80.hint"] = "Also pings you on Telegram as soon as a service reaches 80% usage",
        ["telegram.notify80.message"] = "Heads up, {0} is at 80% 🫣",

        ["popup.noservices"] = "No active services. Enable them from Settings.",
        ["popup.error.generic"] = "Couldn't read usage",
        ["popup.updated"] = "Updated {0}",
        ["popup.resets"] = "Resets {0}",
        ["popup.loading"] = "Updating…",
        ["popup.loading.progress"] = "{0} ({1}/{2})",
        ["popup.tooltip.refresh"] = "Refresh",
        ["popup.tooltip.settings"] = "Settings",
        ["popup.tooltip.stats"] = "Statistics",
        ["popup.tooltip.close"] = "Close panel",
        ["popup.tooltip.pin"] = "Pin panel (won't close on its own)",
        ["popup.tooltip.compact"] = "Compact view",
        ["popup.tooltip.compact.full"] = "Full view",
        ["popup.compact.none"] = "You haven't picked any service for the compact view. Turn it on in Settings.",

        ["stats.title"] = "Statistics",
        ["stats.maximize"] = "Maximize",
        ["stats.restore"] = "Restore",
        ["stats.subtitle"] = "Last few hours",
        ["stats.range.today"] = "Today",
        ["stats.range.yesterday"] = "Yesterday",
        ["stats.range.week"] = "Week",
        ["stats.range.month"] = "Month",
        ["stats.range.custom"] = "Other date",
        ["stats.calendarview"] = "Calendar view",
        ["stats.promptmode.new"] = "New",
        ["stats.promptmode.total"] = "Total",
        ["stats.empty"] = "Not enough data yet. Check back later to see how your usage has been trending.",
        ["stats.reset"] = "✨ Reset",
        ["stats.prompts.legend"] = "prompts (max {0})",
        ["stats.prompts.hover"] = "{0} prompts",
        ["stats.dashboard.prompts"] = "{0} prompts",
        ["stats.dashboard.projects"] = "{0} projects",
        ["stats.dashboard.tasks"] = "{0} tasks",
        ["stats.dashboard.totals.title"] = "Totals (full history)",
        ["stats.dashboard.totals.badge"] = "Total",
        ["stats.legend.usage"] = "AI usage",
        ["stats.legend.prompts"] = "Prompts",
        ["stats.noservices"] = "No active services.",

        ["tray.tooltip.starting"] = "{0}: starting...",
        ["tray.tooltip.noservices"] = "{0}: no active services (right-click → Settings)",
        ["menu.refresh"] = "Refresh now",
        ["menu.settings"] = "Settings...",
        ["menu.about"] = "About...",
        ["menu.login"] = "Sign in",
        ["menu.exit"] = "Exit",
        ["loading"] = "Loading...",

        ["toast.reset"] = "{0} usage has reset ✨{1}",
        ["toast.exhausted"] = "You've run out of {0} 😭{1}",

        ["dialog.yes"] = "Yes",
        ["dialog.no"] = "No",
        ["dialog.later"] = "Not today",
        ["dialog.ok"] = "OK",
        ["dialog.update.title"] = "Update available",
        ["dialog.update.message"] = "A new version is available (v{0}).\nDo you want to update now?",

        ["about.version"] = "Version {0}",
        ["about.path.label"] = "File path",
        ["about.changelog.label"] = "What's new",
        ["about.checkupdate.link"] = "Check for updates",
        ["about.checkupdate.checking"] = "Checking for updates...",
        ["dialog.checkfailed.message"] = "Couldn't check for updates.",
        ["dialog.ratelimited.message"] = "GitHub is temporarily rate-limiting update checks. Try again in a little while.",
        ["dialog.toosoon.message"] = "Already checked a moment ago. Try again in a few seconds.",
        ["dialog.uptodate.message"] = "You're already on the latest version.",

        ["debug.save.label"] = "Save diagnostic data",
        ["debug.save.hint"] = "Records the exact shape of the data coming from Claude/ChatGPT/Grok on your PC on every refresh — only used to investigate bugs like one that shows a limit wrong; never sent anywhere unless you share it with the button below.",
        ["debug.share.button"] = "Save and share logs",
        ["debug.clear.button"] = "Clear diagnostic data",
        ["debug.share.nofiles.title"] = "No diagnostic data",
        ["debug.share.nofiles.message"] = "There's no diagnostic file saved yet.",
        ["debug.share.error.title"] = "Couldn't prepare the file",
        ["debug.share.error.message"] = "Couldn't prepare the file to share:\n{0}",
        ["debug.share.fallback.title"] = "File ready",
        ["debug.share.fallback.message"] = "Windows' share panel couldn't be opened, but the file is ready here so you can share it by hand:\n{0}",
        ["debug.clear.confirm.title"] = "Clear diagnostic data",
        ["debug.clear.confirm.message"] = "This deletes every diagnostic file saved on this PC. It doesn't affect your account or your settings. Continue?",
        ["debug.clear.done.title"] = "Done",
        ["debug.clear.done.message"] = "Diagnostic data cleared.",
        ["debug.clear.error.message"] = "Couldn't fully clear it:\n{0}",

        ["error.startup.body"] = "The application couldn't start.",
        ["error.runtime.body"] = "An unexpected error occurred. The application will keep running, but something may not have worked correctly.",
        ["error.logpath"] = "Details saved to:\n{0}",
        ["startup.folder.readonly.body"] = "This folder isn't writable by your user account:\n{0}\n\nThis will prevent the app from updating itself (and may affect other features, like stats logging). Move the app to another folder — for example, by reinstalling it with the installer into its default folder — or run it as administrator.",

        ["time.available"] = "available now",
        ["time.ago.moment"] = "just now",
        ["time.ago.seconds"] = "a few seconds ago",
        ["time.ago.minutes"] = "{0} min ago",
        ["time.ago.hours"] = "{0} h ago",
        ["time.ago.days"] = "{0} d ago",

        ["provider.claude.5h"] = "5-hour limit",
        ["provider.weekly"] = "Weekly limit",
        ["provider.claude.fable"] = "Fable",
        ["qualifier.weekly"] = "weekly",
        ["qualifier.5h"] = "5 hours",
        ["qualifier.usage"] = "usage",
        ["qualifier.fable"] = "Fable",
        ["prefix.weekly"] = "W:",
        ["prefix.5h"] = "5H:",
        ["prefix.fable"] = "F:",
        ["prefix.usage"] = "U:",
        ["provider.grok.usage"] = "Usage limit",
        ["provider.timeout"] = "Request timed out",
        ["provider.grok.loginneeded"] = "Sign in from the Grok window",
        ["provider.chatgpt.credits"] = "Credit balance: {0:0}",
        ["provider.chatgpt.resets"] = "Resets available: {0}",
        ["provider.chatgpt.weekly_general"] = "General weekly",
        ["provider.chatgpt.weekly_model"] = "{0} weekly",
        ["qualifier.chatgpt.weekly_general"] = "general weekly",
        ["qualifier.chatgpt.weekly_model"] = "{0} weekly",
        ["prefix.chatgpt.weekly_model"] = "W+",
        ["provider.claude.credits.used_of"] = "Credits used: {0:0.00} / {1:0.00} {2}",

        ["telegrambot.prompt"] = "Tap the button or send /uso to check your usage.",
        ["telegrambot.none"] = "No active services yet.",
        ["telegrambot.nodata"] = "no data",
        ["telegrambot.usebutton"] = "Use the \"{0}\" button or the /uso command.",
        ["telegrambot.apps.title"] = "🖥️ Apps in use",
        ["telegrambot.apps.none"] = "No app with an open window was detected.",
        ["telegrambot.apps.foreground"] = "foreground",
        ["telegrambot.apps.active"] = "active ({0}% CPU)",
        ["telegrambot.apps.footer"] = "{0} apps open",
        ["telegrambot.projects.none"] = "No Claude Code history found on this PC.",
        ["telegrambot.projects.active"] = "now",
        ["telegrambot.projects.prompts"] = "{0} prompts",
        ["telegrambot.shutdown.confirm"] = "⚠️ Are you sure you want to *shut down* the PC?",
        ["telegrambot.restart.confirm"] = "⚠️ Are you sure you want to *restart* the PC?",
        ["telegrambot.confirm.yes"] = "✅ Yes",
        ["telegrambot.confirm.no"] = "❌ Cancel",
        ["telegrambot.cancelled"] = "Cancelled.",
        ["telegrambot.shutdown.doing"] = "🔴 Shutting down the PC…",
        ["telegrambot.restart.doing"] = "🔁 Restarting the PC…",
    };
}
